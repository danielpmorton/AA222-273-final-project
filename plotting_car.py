...
def plot_best_filter_vs_truth(times, xHistory, muHistory, sigmaHistory):
    ...
    title:        The title for the overall plot (not the subplots)
    '''
    # Helper function
...
def plot_generation(xHistory, muHistories_history, evalHistory, gen=0):
    ...
    for (i, muHistory) in enumerate(muHistories):
        ...
        plt.plot(muHistory[0,:], muHistory[1,:], c=(rank_normalized, 1-rank_normalized, 0), alpha=0.5, label="All Filters")
    
    # Plotting the best filter in a distinguishable color
    best_muHistory = muHistories[np.argmin(evals)]
    plt.plot(best_muHistory[0,:], best_muHistory[1,:], c='b', label="Best filter")
...
def plot_mahalanobis_convergence(evalHistory):
    ...
    plt.title("Mahalanobis Evaluation Convergence")
...
def plot_chromosome_convergence(bestHistory, trueChromosome):
    ...
    for i in range(chromosome_size): 
        ...
        plt.plot(iterations, trueChromosome[i]*np.ones(len(iterations)), c='k', label="True value")
        plt.plot(iterations, bestHistory_array[:,i], c='r', label="GA result")
        plt.xlabel("Iteration")
        plt.ylabel(f"Chromosome bit {i}")
